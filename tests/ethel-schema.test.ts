...
describe('ethel/lifecycle schema', () => {
  it('inserts a minimal ethel asset with defaults', async () => {
    const [row] = await db.insert(ethelAssets).values({ name: 'Washing machine' }).returning();
    expect(row!.decommissionedAt).toBeNull();
  });

  it('rejects a decommission date without a reason (pair check)', async () => {
    await expect(
      db.insert(ethelAssets).values({ name: 'X', decommissionedAt: '2026-01-01' }),
    ).rejects.toThrow();
  });
  ...
  it('enforces one purchase link per asset (partial unique index)', async () => {
    // Two REAL transactions, one asset, two 'purchase' links: second must fail.
    // Seed a member for created_by via helpers, then:
    const { seedTestHousehold } = await import('./helpers.js');
    const { adult } = await seedTestHousehold();
    const [asset] = await db.insert(ethelAssets).values({ name: 'Z' }).returning();
    const txIds = await db.execute(sql`
      INSERT INTO transactions (date, payee, amount, created_by)
      VALUES ('2026-01-01', 'a', '1.00', ${adult.user.id}::uuid),
             ('2026-01-02', 'b', '2.00', ${adult.user.id}::uuid)
      RETURNING id`) as unknown as Array<{ id: string }>;
    await db.insert(feohItemCosts).values({ transactionId: txIds[0]!.id, assetId: asset!.id, kind: 'purchase' });
    await expect(
      db.insert(feohItemCosts).values({ transactionId: txIds[1]!.id, assetId: asset!.id, kind: 'purchase' }),
    ).rejects.toThrow();
  });
});
