...
describe('inventory routes', () => {
  ...
  it('rejects a limit above the cap with 400 VALIDATION_ERROR', async () => {
    ...
    for (const limit of [101, 200]) {
      const res = await app.request(`/api/v1/inventory/items?limit=${limit}`, { headers: authHeaders(adult.jwt) });
      expect(res.status, `limit=${limit} must be rejected`).toBe(400);
      const body = await res.json() as { error: { code: string } };
      expect(body.error.code).toBe('VALIDATION_ERROR');
    }
    const atCap = await app.request('/api/v1/inventory/items?limit=100', { headers: authHeaders(adult.jwt) });
  ...
  it('paginates with limit/offset and reports total in meta', async () => {
    ...
    for (const name of ['Chisel', 'Anvil', 'Bellows']) {
      await app.request('/api/v1/inventory/items', {
        method: 'POST', headers: authHeaders(adult.jwt), body: JSON.stringify({ name }),
      });
    }

    const first = await app.request('/api/v1/inventory/items?limit=2&offset=0', { headers: authHeaders(adult.jwt) });
    ...
    const second = await app.request('/api/v1/inventory/items?limit=2&offset=2', { headers: authHeaders(adult.jwt) });
  ...
  it('filters by q across name, manufacturer, model and serial, and totals the matches', async () => {
    ...
    for (const [q, expected] of [['bosch', 'Cordless drill'], ['5310', 'Dishwasher'], ['114207', 'Boiler']] as const) {
      const res = await app.request(`/api/v1/inventory/items?q=${q}&limit=50&offset=0`, { headers: authHeaders(adult.jwt) });
