...
import OccurrenceStrip, { OverdueBadge } from '@/components/feoh/occurrence-strip';
...
export default function BillsList({ bills, onRemove }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-end"><OverdueBadge /></div>
      {bills.length === 0 ? (
        <div className="text-sm text-ash py-4 text-center">{t('feoh.noRecurringBills')}</div>
      ) : (
        <ul className="space-y-2">
          {bills.map((b) => (
            <li key={b.id} className="rounded-lg border border-tan bg-card px-3 py-2">
