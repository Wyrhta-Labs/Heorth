    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-parchment" role="dialog" aria-modal="true" aria-label={recipe.title}>
      <div className="flex items-start justify-between gap-4 border-b border-tan px-10 py-6">
        <div>
          ...
          <p className="mt-1 text-xl text-ash">Serves {recipe.servings}</p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="grid flex-1 grid-cols-[1fr_1.6fr] gap-10 overflow-y-auto px-10 py-8">
        <div>
          <h3 className="mb-4 font-serif text-2xl text-ember">Ingredients</h3>
          <ul className="space-y-3">
            ...
            {recipe.ingredients.length === 0 && <li className="text-lg text-ash">No ingredients listed.</li>}
          </ul>
        </div>
        <div>
          <h3 className="mb-4 font-serif text-2xl text-ember">Method</h3>
          <ol className="space-y-5">
            ...
            {recipe.steps.length === 0 && <li className="text-lg text-ash">No method recorded.</li>}
          </ol>
        </div>
      </div>
    </div>
